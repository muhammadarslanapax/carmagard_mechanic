...
import 'package:carmagard/Model/location_cordinates.dart';
import 'package:carmagard/Model/vehicle_model.dart';
import 'package:carmagard/Provider/info_window_provider.dart';
import 'package:carmagard/Service/baseurl.dart';
import 'package:carmagard/Service/map_services.dart';
...
class _PracticePageState extends ConsumerState<MapScreen>
...
//! function to retrieve the autocomplete data from get-places API of google maps
  getSuggestion(String input) async {
    String kPLACES_API_KEY = ApiUrl.mapApiKey;
    String baseURL =
  ...
  final key = ApiUrl.mapApiKey; //!Important!!!<<Place your API key Here>>
  ...
  void setVehicleMarker(Position position) {
    _markers.add(Marker(
      ...
      onTap: () {
        ...
        providerObject.updateVehicle(Vehicle(
            1,
            "Demo Vehicle",
            false,
            "17/9/2023 - 2:21 PM",
              ...
              onCameraMove: (CameraPosition position) {
                ...
                if (providerObject.vehicle != null) {
                  providerObject.updateInfoWindow(
                    context,
                    mapController,
                    LatLng(providerObject.vehicle!.coordinate.latitude,
                        providerObject.vehicle!.coordinate.longitude),
                    _infoWindowWidth,
                    _markerOffset,
                  );
                  providerObject.rebuildInfoWindow();
        ...
        Container(
          ...
          child: TextFormField(
            ...
            onEditingComplete: () async {
              ...
              // }
              String url =
                  "https://maps.googleapis.com/maps/api/geocode/json?latlng=${_position!.latitude},${_position!.longitude}&key=${ApiUrl.mapApiKey}";
              var response = await http.get(Uri.parse(url));
              var res = response.body.toString();
              var body = jsonDecode(res);
              String location = body["results"]["formatted_address"];
              var directions = await MapServices().getDirections(
                location,
...
//!Function to show auto complete suggestion in stack upon origin to destination navigation in flutter
  Positioned showPlacesAutocomplete() {
                            ...
                            if (snapshot.hasData) {
                              return ListTile(
                                ...
                                onTap: () async {
                                  ...
                                  var body = jsonDecode(res);
                                  String location =
                                      body["results"]["formatted_address"];
